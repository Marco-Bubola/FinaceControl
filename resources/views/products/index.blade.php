...
<div class="d-flex justify-content-between align-items-center">
    <div class="row w-100">
                    ...
                    <div class="dropdown-menu p-3 shadow-lg border-0 filtro-dropdown-panel"
                        ...
                        <div class="container-fluid">
                            <div class="row g-2 mb-2 align-items-end">
                                <div class="col-6">
                                    <label class="form-label fw-semibold text-primary mb-1 small"><i
                                            class="bi bi-eye me-1 small"></i>Mostrar:</label>
                                    <div class="btn-group w-100" role="group" aria-label="Mostrar por página">
                                        ...
                                        <input type="radio" class="btn-check" name="per_page" id="per_page_{{ $opt }}"
                                            value="{{ $opt }}" autocomplete="off"
                                            {{ $per_page_sel==$opt?'checked':'' }}>
                                        <label class="btn btn-outline-primary btn-sm px-3 py-1"
                                            for="per_page_{{ $opt }}"><i class="bi bi-list-ol"></i> {{ $opt }}</label>
                                        @endforeach
                                    </div>
                                </div>
                                <div class="col-6">
                                    <label class="form-label fw-semibold text-primary mb-1 small"><i
                                            class="bi bi-sort-alpha-down me-1 small"></i>Ordenar:</label>
                                    <div class="btn-group w-100" role="group" aria-label="Ordenar">
                                        ...
                                        <label class="btn btn-outline-primary btn-sm px-3 py-1"
                                            for="ordem_{{ $key }}"><i class="bi {{ $info['icon'] }}"></i>
                                            {{ $info['label'] }}</label>
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <p class="text-primary fw-bold mb-2 small"><i
                                            class="bi bi-funnel me-1"></i>Informações Gerais</p>
                                    <div class="mb-2">
                                        <label class="form-label fw-semibold text-primary small"><i
                                                class="bi bi-box-seam me-1 small"></i>Tipo:</label>
                                        <div class="btn-group w-100" role="group" aria-label="Tipo de Produto">
                                            ...
                                            <input type="radio" class="btn-check" name="tipo"
                                                id="tipo_{{ $key ?: 'todos' }}" value="{{ $key }}" autocomplete="off"
                                                {{ $tipo_sel===$key?'checked':'' }}>
                                            <label class="btn btn-outline-primary btn-sm px-3 py-1"
                                                for="tipo_{{ $key ?: 'todos' }}">{{ $label }}</label>
                                            @endforeach
                                        </div>
                                    </div>
                                    <div class="mb-2">
                                        <label class="form-label fw-semibold text-primary small"><i
                                                class="bi bi-tags me-1 small"></i>Categoria:</label>
                                    ...
                                    <div class="mb-2">
                                        <label class="form-label fw-semibold text-primary small"><i
                                                class="bi bi-activity me-1 small"></i>Status:</label>
                                        <div class="btn-group w-100" role="group" aria-label="Status">
                                            ...
                                            'ativo' => ['icon' => 'bi-check-circle-fill text-success', 'label' =>
                                            'Ativo'],
                                            'inativo' => ['icon' => 'bi-pause-circle text-warning', 'label' =>
                                            'Inativo'],
                                            'descontinuado' => ['icon' => 'bi-x-circle-fill text-danger', 'label' =>
                                            'Descontinuado'],
                                            ];
                                            @endphp
                                            @foreach ($status_opts as $key => $info)
                                            <input type="radio" class="btn-check" name="status_filtro"
                                                id="status_{{ $key ?: 'todos' }}" value="{{ $key }}" autocomplete="off"
                                                {{ $status_filtro===$key?'checked':'' }}>
                                            <label class="btn btn-outline-primary btn-sm px-3 py-1"
                                                for="status_{{ $key ?: 'todos' }}">
                                    ...
                                    <div class="mb-2">
                                        <label class="form-label fw-semibold text-primary small"><i
                                                class="bi bi-currency-exchange me-1 small"></i>Preço:</label>
                                        <div class="row g-2">
                                            <div class="col-6">
                                                <input type="number" step="0.01" min="0" name="preco_min"
                                                    class="form-control form-control-sm" placeholder="Mínimo"
                                                    value="{{ request('preco_min') }}">
                                            </div>
                                            <div class="col-6">
                                                <input type="number" step="0.01" min="0" name="preco_max"
                                ...
                                <div class="col-md-6">
                                    <p class="text-primary fw-bold mb-2 small"><i class="bi bi-stack me-1"></i>Estoque e
                                        Datas</p>
                                    <div class="mb-2">
                                        <label class="form-label fw-semibold text-primary small"><i
                                                class="bi bi-stack me-1 small"></i>Estoque:</label>
                                        <div class="row g-2 align-items-center">
                                            ...
                                            <div class="col-6">
                                                <select name="estoque" class="form-select form-select-sm">
                                                    ...
                                                    <option value="zerado" {{ $estoque_sel=='zerado'?'selected':'' }}>
                                                        Zerados</option>
                                                    <option value="abaixo" {{ $estoque_sel=='abaixo'?'selected':'' }}>
                                                        Abaixo de</option>
                                                </select>
                                            </div>
                                            <div class="col-6">
                                                <input type="number" min="1" name="estoque_valor"
                                                    class="form-control form-control-sm" placeholder="Qtd"
                                                    value="{{ $estoque_valor }}" @if($estoque_sel!='abaixo' ) disabled
                                                    @endif>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="mb-2">
                                        <label class="form-label fw-semibold text-primary small"><i
                                                class="bi bi-calendar-range me-1 small"></i>Data de Criação:</label>
                                        <div class="row g-2">
                                            <div class="col-6">
                            ...
                            <div class="row">
                                <div class="col-12 d-flex gap-2">
                                    ...
                                    <button type="button"
                                        class="btn btn-outline-secondary flex-fill fw-bold btn-sm py-2"
        ...
        <div class="col-md-6 d-flex justify-content-end align-items-center mb-0 gap-3">
            <button type="button"
                class="btn btn-primary btn-md d-flex align-items-center gap-2 px-4 py-3 shadow rounded-pill fs-7"
                data-bs-toggle="modal" data-bs-target="#modalCriarKit">
                <i class="bi bi-boxes fs-3"></i>
                Novo Kit
            </button>
            <button type="button"
                class="btn btn-success btn-md d-flex align-items-center gap-2 px-4 py-3 shadow rounded-pill fs-7"
                data-bs-toggle="modal" data-bs-target="#modalAddProduct">
                <i class="bi bi-plus-square fs-3"></i>
                Novo Produto
            </button>
            <button type="button"
                class="btn btn-secondary btn-md d-flex align-items-center gap-2 px-4 py-3 shadow rounded-pill fs-7"
    ...
    <div class="col-md-4 mb-4">
        <div class="card border-3 border-primary shadow-lg h-100 position-relative rounded-4 bg-white">
            ...
            <div class="d-flex flex-column align-items-center p-3">
                ...
                <button class="btn btn-outline-primary btn-sm rounded-pill px-3 d-flex align-items-center gap-1 mt-2"
...
.filtro-dropdown-panel .form-control,
.filtro-dropdown-panel .form-select {
    border-radius: 8px;
    font-size: 0.98rem;
