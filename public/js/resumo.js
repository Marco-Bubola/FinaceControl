                ...
                tooltip: {
                    callbacks: {
                        label: function (tooltipItem) {
                            // Corrige erro: tooltipItem.raw pode ser array ou objeto
                            let value = tooltipItem.raw;
                            if (Array.isArray(value)) value = value[tooltipItem.dataIndex];
    ...
    function bindDividirCheckbox() {
        document.querySelectorAll('.dividir-checkbox').forEach(function (checkbox) {
            checkbox.addEventListener('change', function () {
                ...
                // Corrige CSRF token para funcionar fora de Blade
                let csrfToken = document.querySelector('meta[name="csrf-token"]');
                csrfToken = csrfToken ? csrfToken.getAttribute('content') : (window.Laravel && window.Laravel.csrfToken) || '';
                fetch(`/invoices/${invoiceId}/toggle-dividida`, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            console.error('Erro ao dividir fatura:', data.message || data);
                        }
                    })
                    .catch(err => {
                        console.error('Erro ao dividir fatura:', err);
                    });
            });
        });
    }
